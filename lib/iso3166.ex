defmodule ISO3166 do
  ...
  @section "4"
  @revid "690144849"
  @url "https://en.wikipedia.org/w/api.php?action=parse&section=#{@section}&prop=text&format=json&formatversion=2&oldid=#{
         @revid
       }"
  ...
  [{"table", _, [{"tbody", [], rows}]}] =
    @file_cache |> File.read!() |> Floki.parse_document!() |> Floki.find("table.sortable")

  {:ok, region_rows} = @region_file_name |> File.read!() |> ExCsv.parse()

  tuple = fn name, abr2, abr3, num ->
    {name, String.downcase(abr2), String.downcase(abr3), String.to_integer(num)}
  end
  ...
  @mappings Enum.reduce(rows, [], fn row, acc ->
              case row do
                ...
                {"tr", [],
                 [
                   {"td", _, [{"span", _, [{"a", _, [name]}]}]},
                   {"td", _, [{"a", _, [_, {"span", _, [abr2]}]}]},
                   {"td", _, [_, {"span", _, [abr3]}]},
                   {"td", _, [_, {"span", _, [num]}]},
                   {"td", _, _}
                 ]} ->
                  [tuple.(name, abr2, abr3, num) | acc]
  ...
  IO.puts("There are #{length(@mappings)} ISO-3166 codes")
  ...
  Enum.each(@mappings, fn {_name, abr2, abr3, _num} ->
  ...
  # numeric abreviation with a catchall that returns :undefined
  Enum.each(@mappings, fn {_name, abr2, _abr3, num} ->
    defp do_cc2to_num(unquote(abr2)), do: unquote(num)
  end)

  defp do_cc2to_num(_), do: :undefined

  # generate the functions that map 3 letter abbreviation to 2
  # letter abreviation with a catchall that returns :undefined
  Enum.each(@mappings, fn {_name, _abr2, abr3, num} ->
    defp do_cc3to_num(unquote(abr3)), do: unquote(num)
  end)

  defp do_cc3to_num(_), do: :undefined

  # generate the functions that map 2 letter abbreviation to
  # name with a catchall that returns :undefined
  Enum.each(@mappings, fn {name, abr2, _abr3, _num} ->
  ...
  # name with a catchall that returns :undefined
  Enum.each(@mappings, fn {name, _abr2, abr3, _num} ->
  ...
  def cc2to_num(v), do: do_with_matching_case(v, &do_cc2to_num/1)

  def cc3to_num(v), do: do_with_matching_case(v, &do_cc3to_num/1)
